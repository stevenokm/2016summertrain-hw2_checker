...
command			: NAMES { id_list.clear(); names_count++; } ids ID output_covers
				{
					...
					for(output_it = output_cover_list.begin(); output_it != output_cover_list.end(); output_it++) {
						it = id_list.begin();
						if(output_it->second.compare("1") == 0) {
							...
							for(string::iterator char_it = output_it->first.begin(); char_it != output_it->first.end(); char_it++) {
								if(*char_it == '0') {
									function_file << ' ' << **it << "\'";
								} else if(*char_it == '1') {
									function_file << ' ' << **it;
